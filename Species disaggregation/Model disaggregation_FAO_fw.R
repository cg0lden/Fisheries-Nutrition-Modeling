...
##Nutrients
mar_fw_spp_nutrients <- read_csv("data/mar_fw_spp_nutrients_FAO_fw.csv") %>% 
  group_by(common_name, nutrient) %>% 
  summarise(value = mean(value)) %>% 
  ungroup() %>% 
  drop_na(common_name)

mar_fw_spp_nutrients_categories <- read_csv("data/mar_fw_spp_nutrients_FAO_fw.csv") %>% 
  dplyr::select(common_name, nutrient, category, value) %>% 
  group_by(category, nutrient) %>% 
  summarise(value = mean(value)) %>% 
  drop_na(category)

mar_fw_spp_nutrients_broad_categories <- read_csv("data/mar_fw_spp_nutrients_FAO_fw.csv") %>% 
...
#   group_by(iso3c, year, env, units, scenario) %>%
#   summarise(pred_consumption = sum(value2)) %>%
#   spread(key = env, value=pred_consumption) %>%
...
#          iso3c=="BRA") 
# 
# FC_fish_check = FC_fish %>%
#   filter(year == 2030,
#          scenario == "base",
#          nutrient=="Calcium",
#          iso3c=="BRA")
# 
# spp_prop_check = spp_prop %>%
#   filter(iso3c=="BRA") %>%
#   group_by(production_sector, env) %>%
#   summarise(spp_prop = sum(spp_prop))
# 
# QP_Prop_check = QP_prop %>%
#   filter(year == 2030,
#          scenario == "base",
#          iso3c=="BRA")
...
NutrientsScen_long_dis_short_2030 = NutrientsScen_long_dis_short %>% 
  filter(year==2030)
ggplot(data = NutrientsScen_long_dis_short_2030) +
  geom_point(aes(x=nutrient_supply, y=nutrient_supply_dis)) +
  facet_wrap(~nutrient, scales = "free") +
  geom_abline(intercept = 0, slope = 1) +
  ylab("Nutrient supply - disaggregation")+
