...
##Exclude other fish parts (liver, overy, eggs, etc)
AFCD_long = AFCD_long %>% 
  filter(Parts %in% AFCD_parts)

# Iron: 0-15 mg/100g (based on the boxplot of most common values)
# Zinc: 0-5 mg/100g (based on the boxplot of most common values)
# Protein: 0-40 g/100g
# Vitamin A: 0-9 mg/100g
# Vitamin B12: 0.7-12 ug/100g
# Omega3: 0-3.5 g/100g
# Calcium: 0-3000 mg/100g (although most values are below 300)

##Exclude outliers
AFCD_long$value[AFCD_long$variable=="Calcium" & AFCD_long$value>3000]=NA
AFCD_long$value[AFCD_long$variable=="Iron" & AFCD_long$value>15]=NA
AFCD_long$value[AFCD_long$variable=="Vitamin A" & AFCD_long$value>9]=NA
AFCD_long$value[AFCD_long$variable=="Vitamin B12" & AFCD_long$value>12]=NA
AFCD_long$value[AFCD_long$variable=="Omega-3 fatty acids" & AFCD_long$value>4]=NA
AFCD_long$value[AFCD_long$variable=="Zinc" & AFCD_long$value>5]=NA
AFCD_long$value[AFCD_long$variable=="Protein" & AFCD_long$value>40]=NA
...
# 
# y = ggplot(data=AFCD_long)+
#   geom_boxplot(aes(y=value))+
#   facet_wrap(~variable, scales = "free")
# 
# ggsave(y, file="AFCD_boxplot_removed_outliers.pdf")
# 
# nut = unique(AFCD_long$variable)
# for(n in 1:length(nut)){
#   df_nut = as.numeric(df$qt[df$variable==nut[n]][4])
#   x = ggplot(data = AFCD_long %>% filter(variable == nut[n])) +
#     geom_boxplot(aes(y = value))+
#     ylim(0, df_nut)
#   ggsave(x, file=paste0("boxplot_", nut[n],".pdf"))
# }
...
write.csv(sau_nutrition, "data/mar_fw_spp_nutrients_FAO_fw.csv", row.names = F)
...
ggplot(data=fill_stats)+
  ...
  labs(title="(2,143 unique species)", fill="N") +
  ylab("")+
  xlab("Nutrient")+
